...
module.exports = function(schema, options) {
  options = options || {};
  options.v2MigrationFlag = options.v2MigrationFlag || 'v2MigrationFlag';
  ...
  var pbkdf2 = function(password, salt, migrated, callback) {
    if(migrated && crypto.pbkdf2.length >= 6) {
  ...
  if (!schema.path(options.usernameField)) {
    schemaFields[options.usernameField] = {type: String, unique: options.usernameUnique, sparse:true};
  }
  ...
  schema.methods.setPassword = function(password, cb) {
    ...
    options.passwordValidator(password, function(err) {
      ...
      crypto.randomBytes(options.saltlen, function(err, buf) {
        ...
        pbkdf2(password, salt, true, function(err, hashRaw) {
          if (err) {
            return cb(err);
          }
  ...
  function authenticate(user, password, cb) {
    ...
    pbkdf2(password, user.get(options.saltField), user.get(options.v2MigrationFlag), function(err, hashRaw) {
      if (err) {
        return cb(err);
      }
      ...
      if (scmp(hash, user.get(options.hashField))) {
        ...
        if(!user.get(options.v2MigrationFlag)) {
          user.setPassword(password,function() {
             user.set(options.v2MigrationFlag,true);
